...
exports.getAllLeads = async (req, res) => {
  try {
    ...
    console.log('[LEADS API] GET all leads - Query params:', { stage, source, priority, assignedSalesEngineer, search, sortBy, page, limit });
    ...
    console.log('[LEADS API] Found', leads.length, 'leads, Total:', totalLeads, 'Filter:', filter);
...
exports.getMyAssignedLeads = async (req, res) => {
  try {
    ...
    console.log('[LEADS API] ===== GET MY ASSIGNED LEADS =====');
    console.log('[LEADS API] Logged-in user ID:', userId);
    console.log('[LEADS API] User role:', req.user.role);

    const leads = await Lead.find({ assignedSalesEngineer: userId })
      .populate('createdBy', 'firstName lastName email name')
      .sort({ createdAt: -1 });

    console.log('[LEADS API] Query filter:', { assignedSalesEngineer: userId });
    console.log('[LEADS API] Found', leads.length, 'assigned leads');
    
    if (leads.length > 0) {
      console.log('[LEADS API] First lead:', {
        id: leads[0]._id,
        name: leads[0].firstName + ' ' + leads[0].lastName,
        assignedTo: leads[0].assignedSalesEngineer
      });
    }
